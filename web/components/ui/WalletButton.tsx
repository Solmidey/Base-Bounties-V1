                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        disabled && !isLoading ? 'cursor-not-allowed opacity-50' : 'hover:border-[#00ffd1]/50 hover:bg-white/[0.08]',
                      )}
                    >
                      <div className="relative flex items-center justify-between gap-3">
                        <div className="flex flex-col">
                          ...
                          {connector.ready ? (
                            <span className="text-xs text-white/60">{address ? 'Switch wallet' : 'Connect instantly'}</span>
                          ) : (
                            <span className="text-xs text-amber-300/80">Install to enable</span>
                          )}
                        </div>
                        {isLoading ? (
                          ...
                          <span className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs text-white/70 transition group-hover:bg-[#00ffd1]/20 group-hover:text-white">
                            {connector.type === 'injected' ? 'Browser' : 'External'}
                          </span>
                        )}
                      </div>
                    </button>
